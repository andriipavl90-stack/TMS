<template>
  <div class="report-view">
    <div class="report-card">
      ...
      <div v-if="state === 6" class="status-badge status-approved">Approved</div>
      <div v-if="state === 5" class="status-badge status-pending">Awaiting Manager</div>
      <div v-if="state === 4" class="status-badge status-rejected">Rejected</div>
...
async function loadReport() {
  try {
    ...
    if (props.p_id && props.p_id !== '') {
      ...
      item = response.data.find((r) => r.date.split('T')[0] === date.value && r.state > 4);
    }
    if (!item) return;
